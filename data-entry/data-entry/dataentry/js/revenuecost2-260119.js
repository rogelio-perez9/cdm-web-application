...
function calcBamt(nn) {
	...
	const currency = $(`#curr${nn} option:selected`).text().trim(); // get's CURRENCY
...
function calcVbamt(nn) {
	...
	const currency = $(`#vcurr${nn} option:selected`).text().trim(); // get's CURRENCY
	const vexrate = Number($('#vexrate').val());

	if (currency === 'USD' && (vexrate > 0) && cost !== 0) {
		$(`#vexrate${nn}`).val(vexrate.toFixed(4));
