...
function initAutoComplete(config) {
	...
	$("#" + config.fieldId).tokenInput(config.url + "?task=" + config.task + "&sqid=" + sqid + "&complianceid=" + complianceid + query, {
...
function initCombineAutoComplete(config, params = '') {
	...
	$("#" + config.fieldId).tokenInput(function () {
		if (requestUrl.indexOf("getStation2") !== -1) {
			const base = requestUrl;
			const sep = base.indexOf("?") >= 0 ? "&" : "?";
			return base + sep + "_ts=" + Date.now(); // unique per request
		} else {
			return requestUrl;
		}
	},
		{
			...
			onSend: function (g) {
				if (requestUrl.indexOf("getStation2") !== -1) {
					g.data.soffid2 = $("#soffid2").val();
				} else if (requestUrl.indexOf("getPurchaseOrderNo") !== -1) {
					g.data.custid = $("#custid").val();
				}
			},
			onAdd: function (data) {
...
$(document).ready(function () {
	...
	$("#etd1").datepicker({
		dateFormat: "mm/dd/yy",
		onClose: (d, o) => {
			if ((d?.length > 0) && (clientid == "topoceanusa" || clientid == "topoceanasia" || clientid == "topousa" || clientid == "topoasia")) {
	...
	$("#retd1").datepicker({
		...
		onClose: (d, o) => {
			if (d?.length > 0) {
				let date = $("#retd1").datepicker('getDate')
				let woy = getWeekOfYear(date, 1)
				$('#rweek1').val(woy.week)
				$('#ryear1').val(woy.year)
			}
	...
	$("#retd2").datepicker({
		...
		onClose: (d, o) => {
			if (d?.length > 0) {
				let date = $("#retd2").datepicker('getDate')
				let woy = getWeekOfYear(date, 1)
				$('#rweek2').val(woy.week)
				$('#ryear2').val(woy.year)
			}
	...
	$("#v2atd1").datepicker({
		...
		onClose: (d, o) => {
			if (d?.length > 0) {
				let date = $("#v2atd1").datepicker('getDate')
				$("#v2onb1").datepicker('setDate', date)
			}
		},
	})
	...
	const airportConfigByCode = {
		...
		container: 'polcode_th',
		fields: [
			{ ui: 'apolname', api: 'expand' },
			{ ui: 'apolcode', api: 'code' }
		]
	}
	initCombineAutoComplete(airportConfigByCode)
	...
	/* Airport of arrival  */
	const airdischargeConfigByCode = {
		url: 'dataentry.ashx',
		task: 'getUncodesAir',
		queryParam: 'q',
		fieldId: 'apoulcode',
		...
		container: 'poulcode_th',
		fields: [
			{ ui: 'apoulname', api: 'expand' },
			{ ui: 'apoulcode', api: 'code' }
		]
	}
	initCombineAutoCompletePoulcode(airdischargeConfigByCode)
	...
	*/
	...
	initCombineAutoComplete(consigneeConfig2)
	...
	/*  Principal for air */
	const principal2Config = {
		url: 'dataentry.ashx',
		task: 'getContactsPrincipal2',
		queryParam: 'q',
		fieldId: 'prinid2',
		tokenId: 'custid',
		searchKey: 'str',
	...
	initCombineAutoComplete(principal2Config)
	...
	/*  Air Carrier */
	const aircarrierConfig = {
		url: 'dataentry.ashx',
		task: 'getContactsAirCarrier',
		queryParam: 'q',
		fieldId: 'aircarr1id',
		tokenId: 'custid',
		searchKey: 'str',
		minChars: 2,
		preventDuplicates: false,
		container: 'aircarr1id_th',
		fields: [
			{ ui: 'aircarr1name', api: 'custname' },
			{ ui: 'aircarr1id', api: 'custid' },
			{ ui: 'pfx', api: 'prefix' },
			{ ui: 'by01', api: 'iata' }
		]
	}
	initCombineAutoComplete(aircarrierConfig)
	...
	const rptLotno = {
		...
		minChars: 2,
		preventDuplicates: true,
		container: 'rptlotno_th',
		fields: [
			{ ui: 'rptlotno', api: 'manifest' }
		]
	}
	initCombineAutoComplete(rptLotno)
	...
	/* MRUPD.html - Purchase Order No. */

	const purchaseOrderNo = {
		url: 'dataentry.ashx',
		task: 'getPurchaseOrderNo',
		queryParam: 'q',
		fieldId: 'mrpono',
		tokenId: 'pono',
		searchKey: 'str',
		...
		fields: [
			{ ui: 'mrpono', api: 'pono'},
			{ ui: 'consid', api: 'consid', type: 'ti' },
			{ ui: 'consname', api: 'consname' },
			{ ui: 'consref', api: 'consref' },
			{ ui: 'suppid', api: 'suppid', type: 'ti' },
			{ ui: 'suppname', api: 'suppname' },
			{ ui: 'suppref', api: 'suppref' },
			{ ui: 'destcode', api: 'destcode', type: 'ti' },
			{ ui: 'destname', api: 'destname' },
		]
	}
	initCombineAutoComplete(purchaseOrderNo)
...
// Check StationId value
$(document).ready(function () {
	// Watch for changes in #prinid
	$("#prinid").on("change", function () {
		const soffid2Val = $("#soffid2").val();
		...
		if (validValues.includes(soffid2Val)) {

			const station1Val = $('#station1').val();
			const station2Val = $('#station2').val();

			// Build query params
			const q = {
				task: "checkStationValueByStationId",
				sqid: "compliance",
				station1: station1Val,
				station2: station2Val
			};
			...
			$.ajax({
				...
				success: function (response) {
					if (!response) return;
					handlePopUps(response, station1Val, station2Val);
				},
...
async function handlePopUps(response, station1Val, station2Val) {
	// If station1 needs popup, show it and wait for closure
	if (response.station1.Item2 === 1) {
		const closed = await openStationPopUp(station1Val, response.station1.Item1, 1);
		// After it closes, wait 1.5s, then (if needed) show station2
		if (closed && response.station2.Item2 === 1) {
			await delay(1500);
			await openStationPopUp(station2Val, response.station2.Item1, 2);
		}
		return;
	}

	// If station1 doesn't need popup but station2 does, show station2 immediately
	if (response.station2.Item2 === 1) {
		await openStationPopUp(station2Val, response.station2.Item1, 2);
	}
}

function openStationPopUp(station, compname, stationNumber) {
	// If a modal is already open, resolve immediately (avoid stacking)
	if (document.getElementById("openStationPopUpDiv")) {
		return Promise.resolve(false);
	}

	return new Promise((resolve) => {
		const overlay = document.createElement("div");
		overlay.id = "openStationPopUpDiv";
		overlay.innerHTML = `
      <div class="station-modal" role="dialog" aria-modal="true" aria-labelledby="openStationModalTitle" tabindex="-1">
        <div class="modal-header">
          <h2 id="openStationModalTitle">Station</h2>
        </div>
        ...
        <div class="confirm-row">
          <label class="confirm-label">
            <table class="notes-table" aria-label="Previous notes">
              <tr>
                <td>
                  <span class="confirm-text"><strong>${station}</strong> is not valid</span>
                </td>
              </tr>
              <tr>
                <td>
                  <span class="confirm-text"><strong>${compname}</strong></span>
                </td>
              </tr>
            </table>
          </label>
        ...
        <div class="btn-row">
          <button id="returnBtn" class="btn btn-primary">Return</button>
        </div>
      </div>
    `;
      ...
      .btn-row { display:flex; gap:10px; margin-top: 14px; justify-content: center; }
      .btn {
        min-width: 120px; padding: 10px 14px; border: 1px solid #666; border-radius: 4px;
        cursor: pointer; font-weight: 600; color: #222; background: #f7f7f7;
    ...
    `;
		...
		const modalEl = overlay.querySelector(".station-modal");
		modalEl.addEventListener("click", e => e.stopPropagation());
		overlay.addEventListener("click", () => { });

		function closeModal() {
			$(document).off("keydown.stationEsc");
			overlay.remove();
			style.remove();
		}

		// Block ESC close
		$(document).on("keydown.stationEsc", (e) => {
			if (e.key === "Escape") e.preventDefault();
		});

		$("#returnBtn").on("click", function () {
			$(`#station${stationNumber}name`).val('');
			$(`#station${stationNumber}`).tokenInput('clear');
			$(`#station${stationNumber}`).val('');
			closeModal();
			resolve(true);
		});
	});
